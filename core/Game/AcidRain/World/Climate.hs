...
-- | The climate vector.
newtype instance UV.MVector σ Climate = MV_Cli (UV.MVector σ (Float, Float))
newtype instance UV.Vector    Climate = V_Cli  (UV.Vector    (Float, Float))
...
instance GMV.MVector UV.MVector Climate where
  ...
  basicUnsafeRead (MV_Cli v) i
    = do (temp, hum) ← GMV.basicUnsafeRead v i
         return $ Climate temp hum
  basicUnsafeWrite (MV_Cli v) i cli
    = GMV.basicUnsafeWrite v i (cliTemperature cli, cliHumidity cli)
...
instance GV.Vector UV.Vector Climate where
  ...
  basicUnsafeIndexM (V_Cli v) i
    = do (temp, hum) ← GV.basicUnsafeIndexM v i
         return $ Climate temp hum
